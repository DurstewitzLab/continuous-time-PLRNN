...
function plot_predictions(X, X̃; epoch = nothing)
    if size(X, 1) == 1
        return plot_reconstruction_series(X[1, 1, 2:end], X̃[1, 1, :])
    elseif size(X, 1) == 2
        return plot_predictions_2d(X, X̃; epoch = epoch)
    elseif size(X, 1) == 3
        return plot_predictions_3d(X, X̃; epoch = epoch)
    else
        plot_reconstruction_series(X[1, 1, 2:end], X̃[1, 1, :])
    end
end

function plot_predictions_3d(X::AbstractMatrix, X̃::AbstractMatrix; epoch = nothing)
    plt = plot(
        X[:, 1],
        X[:, 2],
        X[:, 3],
...
function plot_predictions_3d(X::AbstractArray{T,3}, X̃::AbstractArray{T,3}; epoch = nothing) where T
    # X: (N, S, T)
    X_ = permutedims(X, (3, 2, 1))
    X̃_ = permutedims(X̃, (3, 2, 1))
    plt = plot(
        X_[:, :, 1],
        X_[:, :, 2],
        X_[:, :, 3],
...
function plot_predictions_2d(X, X̃; epoch = nothing)
    # X: (N, S, T)
    X_ = permutedims(X, (3, 2, 1))
    X̃_ = permutedims(X̃, (3, 2, 1))
    plt = plot(
...
function plot_reconstruction_series(X::AbstractVector, X̃::AbstractVector)
    @assert size(X, 1) == size(X̃, 1)
    t = 1:size(X, 1)
    fig = plot(t, X, label = "true", legend = true)
    plot!(fig, t, X̃, label = "generated", xlabel = "t", ylabel = "a.u.")
    return fig
end
...
function plot_trajectory(X::AbstractVector, X̃::AbstractVector)
    @assert size(X, 1) == size(X̃, 1)
    t = 1:size(X, 1)
    fig = plot(t, X, label = "true", legend = true)
    plot!(fig, t, X̃, label = "generated", xlabel = "t", ylabel = "a.u.")
...
function plot_trajectory(X::AbstractMatrix, X̃::AbstractMatrix)
    @assert size(X, 2) == size(X̃, 2) == 1
    t = 1:size(X, 1)
    fig = plot(t, X[:, 1], label = "true", legend = true)
    plot!(fig, t, X̃[:, 1], label = "generated", xlabel = "t", ylabel = "a.u.")
